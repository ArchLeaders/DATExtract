...
namespace DATLib
{
    class BMSInterfacer
    {
        ...
        internal static byte[] SendToProcess(byte[] magicBytes, uint decompressedSize, byte[] buffer)
        {
            CheckMutex();
            ...
            Array.Copy(buffer, 0, toWrite, 12, buffer.Length);
            ...
            using (MemoryMappedFile file = MemoryMappedFile.CreateOrOpen("DATManQuickBMS", 32768 + 8, MemoryMappedFileAccess.ReadWrite, MemoryMappedFileOptions.None, HandleInheritability.Inheritable))
            {
                ...
                using (MemoryMappedFile output = MemoryMappedFile.OpenExisting("QuickBMSDATMan"))
                {
                    ...
                    {
                        fileData = new byte[decompressedSize];
                        Console.WriteLine(fileData.Length);
                        outputStream.Read(fileData, 0, (int)decompressedSize);
                    }
                }
                Console.WriteLine(fileData.Length);
            }
